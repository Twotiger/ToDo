      ...
      <transition enter-active-class="animate__fadeInRight">
        <div class="sidebar" v-if="showMenu">
          ...
          <div class="sidebar-content">
            <router-link :to="{ path: '/' }" class="todayToolbar tollbar"
            ...
            <router-link :to="{ path: '/important' }" class="todayToolbar tollbar" :class="
            ...
            <router-link :to="{ path: '/tasks' }" class="todayToolbar tollbar"
          ...
          <div class="task-groups">
            <template v-for="(item, index) in taskGroups" :key="item.id">
              <div class="task-group drag" :class="dropClass(item.id)" draggable="true" @dragover.prevent
                @dragend="dragend" @dragenter="dragenter($event, item.id)" @dragleave="dragleave($event, item.id)"
                @dragstart="dragstart($event, item.id)" @drop="drop($event, item.id)" v-if="item.task_type === 'list'">
                <TaskList :data="item" />
              </div>

              <div v-else draggable="true" @dragover.prevent @dragend="dragend" @dragenter="dragenter($event, item.id)"
                @dragleave="dragleave($event, item.id)" @dragstart="dragstart($event, item.id)"
                @drop="drop($event, item.id)">
                <TaskGroup @dragstart="dragstart" @drop="drop" @dragenter="dragenter" @dropToEmpty="dropToEmpty"
                  v-model:openGroup="openGroup" @dragleave="dragleave" :data="item" />
              </div>
            </template>
          </div>
          ...
          <div class="add-task-group" v-if="willAddTaskGroup">
            <div class="input-group">
              ...
              <input v-model="willAddTaskGroupName" v-focus class="text" @blur="blur" @keyup.enter="addGroupInput" />
            </div>
          </div>
          ...
          <div class="add-task-group">
            <div class="input-group">
              <svg-icon name="plus" />
              <input class="text" v-model="willAddTaskName" @keyup.enter="addTaskList" />
            </div>
            <svg-icon name="add" class="icon pd12 add-group" @click="addGroupInput" />
          </div>
        </div>
      </transition>

      <div class="main">
        <router-view v-model:showMenu="showMenu" @updateCount="getTaskGroup"></router-view>
      </div>
    </div>
  </div>
</template>
